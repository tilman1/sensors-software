...
// wifi credentials
bool ethernet_enabled = ETHERNET_ENABLED;
...
static void WiFiEvent( WiFiEvent_t event)
{
  ...
  switch (event) {
    case SYSTEM_EVENT_ETH_START:
      debug_outln_info(F("ETH Started"));
      //set eth hostname here
      ETH.setHostname(cfg::fs_ssid);
      break;
    case SYSTEM_EVENT_ETH_CONNECTED:
      debug_outln_info("ETH Connected");
      break;
    case SYSTEM_EVENT_ETH_GOT_IP:
      buf = ("ETH MAC: ") + ETH.macAddress() + ", IPv4: " + ETH.localIP() + ", " + ETH.linkSpeed() + "Mbps";
      if (ETH.fullDuplex()) {
        buf += ", FULL_DUPLEX";
      }
      debug_outln_info(buf);
      eth_connected = true;
      break;
    case SYSTEM_EVENT_ETH_DISCONNECTED:
      debug_outln_info(F("ETH Disconnected"));
      eth_connected = false;
      break;
    case SYSTEM_EVENT_ETH_STOP:
      debug_outln_info(F("ETH Stopped"));
      eth_connected = false;
      break;
    default:
      break;
...
static void webserver_config_send_body_get(String& page_content) {
  ...
  add_form_checkbox(Config_ethernet_enabled, FPSTR(INTL_ENABLE_ETHERNET));
...
#if defined (ESP32)
  // turn on ethernet if not already turned on
  if (cfg::ethernet_enabled == false)
  { 
...
static boolean waitForNetworkToConnect(int maxRetries) {
  int retryCount = 0;
  boolean rc = false;
...
static void connectNetwork() {
  ...
  if (cfg::ethernet_enabled == false) {
    ...
    debug_outln_info(F("Turn on Wifi(STA-Mode)"));
    WiFi.begin(cfg::wlanssid, cfg::wlanpwd); // Start WiFI
    debug_outln_info(FPSTR(DBG_TXT_CONNECTING_TO), cfg::wlanssid);
#if defined(ESP32)
  } else {
    // start ethernet
    debug_outln_info(F("Turn on Ethernet"));
    WiFi.onEvent(WiFiEvent);
    ETH.begin();
  }
#endif
...
#if defined(ESP32)
  if (eth_connected == true) {
   debug_outln_info(F("Ethernet connected, Ip is: "), ETH.localIP().toString());
...
#endif
  debug_outln_info(F("init_config"));
  init_config(); // setups flash file system
  debug_outln_info(F("InitDisplay"));
  init_display();

  debug_outln_info(F("connectNetwork"));
  connectNetwork();
  debug_outln_info(F("setupNetworkTime"));
  setupNetworkTime();
  debug_outln_info(F("setup_webserver"));
  setup_webserver();
  debug_outln_info(F("createLoggerConfigs"));
  createLoggerConfigs();
  debug_outln_info(F("\nChipId: "), esp_chipid);
  debug_outln_info(F("\nMAC Id: "), esp_mac_id);
...
void loop(void) {
  ...
  if (send_now) {
    ...
    // reconnect to WiFi if Wifi enabled and disconnected
    if (cfg::ethernet_enabled == false) {
      if (WiFi.status() != WL_CONNECTED) {
       debug_outln_info(F("Connection lost, reconnecting "));
        WiFi_error_count++;
       WiFi.reconnect();
       waitForNetworkToConnect(20);
      }
    }
